...
現在のプロキシは単一の UDP ソケット（`UdpTransport::bind` の `count=1`）で送受信を行っており、高負荷時に送信バッファが溢れて `EMSGSIZE` エラーが発生する（640 CPS でエラー率 48%）。受信ループの並列化（proxy-parallel-recv）は完了済みだが、全ての受信タスクが同一ソケット（`socket_idx=0`）を使用しているため、単一ソケットの送信バッファがボトルネックとなっている。
